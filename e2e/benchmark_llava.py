...
from e2e.quantized_llava import modeling_llava
import torch
import transformers
...
def get_model_quantized(config_name):
    ...
    with transformers.modeling_utils.no_init_weights(): 
        model = modeling_llava.QuarotLlamaForCausalLM(config=config)
...
def get_model_fp16(config_name):
    return modeling_llava.QuarotFP16LlamaForCausalLM.from_pretrained(
        config_name, 
        torch_dtype=torch.float16, 
        attn_implementation="flash_attention_2"
    )
