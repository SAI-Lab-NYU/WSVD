...
@triton.jit
def _fwd_kernel_flash_latent_decode_rope_stage1(
    Q, latent_K, latent_V, wk, sm_scale, Req_to_tokens, B_req_idx, B_Seqlen,
    ...
    for start_n in range(0, block_n_size, 1):
        ...
        # RoPE must use logical sequence positions, not physical cache slots.
...
@torch.no_grad()
def flash_latent_per_head_decode_rope_stage1(q, latent_k, latent_v, wk, Req_to_tokens, B_req_idx, B_Seqlen, max_len_in_batch, mid_out, mid_out_logsumexp, block_seq):
    ...
    head_dim = q.shape[-1]
    head_num = q.shape[1]
    rank_k = latent_k.shape[-1] # latent_k: (batch*seqlen, head_num, rank_k), rank_k per head
    rank_v = latent_v.shape[-1] # latent_v: (batch*seqlen, head_num, rank_v), rank_v per head
    assert head_dim * q.shape[1] == wk.shape[1]  # wk: (rank, headdim*nhead)
    assert rank_k == wk.shape[0]
    # assert rank_v == wv.shape[0]
    ...
    gqa_group_size = 1  # latent is already split by head, so gqa_group_size=1

    _fwd_kernel_flash_latent_decode_rope_stage1[grid](
        q, latent_k, latent_v, wk, sm_scale, Req_to_tokens, B_req_idx, B_Seqlen,
        mid_out,
        mid_out_logsumexp,
