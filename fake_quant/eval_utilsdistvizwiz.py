...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Fix: check whether model_config has mm_use_im_start_end
        # If not, default to not using im_start_end tokens
        use_im_start_end = getattr(self.model_config, "mm_use_im_start_end", False)
...
def evaluator(model, testloader, device, args, tokenizer, image_processor):
    ...
    # Try reading the full JSON file
    # with open(os.path.expanduser(question_file), "r") as f:
    #     questions = json.load(f)
    #     # If questions is not a list, convert it to a list
