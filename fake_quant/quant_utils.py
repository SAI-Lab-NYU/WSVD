...
def symfp8_quant(x, scale, maxq):
    scale = scale.to(x.device)
    q = torch.clamp(x/scale, -maxq, maxq).to(torch.float8_e4m3fn)
    return q, scale
def symfp8_dequant(q, scale):
    return scale * q.float()

def symfp8_quant_dequant(x, scale, maxq):
    return symfp8_dequant(*symfp8_quant(x, scale, maxq))

def get_minq_maxqfp8(bits, sym):
    maxq = torch.tensor(448)
    minq = -torch.tensor(448)
...
class ActQuantizer(torch.nn.Module):
    ...
    def find_params_per_token_groupwise(self, x):
        ...
        #[FIXME: group quant for vit] RuntimeError: shape '[-1, 577, 1, 2752]' is invalid for input of size 2363392
...
class ActQuantWrapper(torch.nn.Module):
    ...
    def __init__(self, module:torch.nn.Linear):
        ...
        self.in_features = module.in_features # for convinence of localft in/out channel 
        ...
        self.online_full_hadnopow2 = False
    ...
    def forward(self, x):
        ...
        if self.fuse_linear:
            # breakpoint()
            x = self.bias_param(x.float()).to(x_dtype)
        if self.fuse_bias:
            a, b = self.bias_param
            x = (x + a * x + b).to(x_dtype)
        if self.online_random_had:
            if x.shape[-1] != self.had_K.shape[0]:
                init_shape = x.shape
                x = x.reshape((init_shape[0], -1, init_shape[-1]//self.had_K.shape[0],self.had_K.shape[0]))
                if self.fp32_had:
...
class WeightQuantizer(torch.nn.Module):
    ...
    def find_paramsb(self, x):
        if self.bits == 16:
            return
        dev = x.device
        self.maxq = self.maxq.to(dev)

        shape = x.shape  # [bsz, channel, in_channel]

        # Reshape for per-channel processing
        if self.perchannel:
            x = x.reshape(x.shape[0], x.shape[1], -1)  # Flatten last two dims
        else:
            x = x.flatten(start_dim=1).unsqueeze(1)  # Flatten everything except batch dim

        # Compute min/max along last dimension (in_channel)
        tmp = torch.zeros(x.shape[:-1], device=dev)  # Shape: [bsz, channel]
        xmin = torch.minimum(x.min(dim=-1)[0], tmp)  # Min along last dimension
        xmax = torch.maximum(x.max(dim=-1)[0], tmp)  # Max along last dimension
        ...
        if self.mse:
            best = torch.full(xmin.shape, float('inf'), device=dev)  # Shape: [bsz, channel]
            for i in range(int(self.maxshrink * self.grid)):
                ...
                q.abs_()
                q.pow_(self.norm)
                err = torch.sum(q, dim=-1)  # Sum over last dimension (in_channel)
                tmp = err < best
                if torch.any(tmp):
                    best[tmp] = err[tmp]
        ...
        if not self.perchannel:
            tmp = shape[1]  # Channel dimension
            self.scale = self.scale.repeat(1, tmp)  # Repeat over batch
            self.zero = self.zero.repeat(1, tmp)

        shape = [shape[0], shape[1]] + [1] * (len(shape) - 2)  # [bsz, channel, 1]
...
def add_actquant_mm(module, name='', layers=[torch.nn.Linear,
                                          ActQuantWrapper,
                                          transformers.models.falcon.modeling_falcon.FalconLinear]):
    if isinstance(module, ActQuantWrapper):
        return
    
    if type(module.mm_projector) == torch.nn.Sequential:
        replaced = []
        for i, child in enumerate(module.mm_projector.children()):
            if type(child) in layers:
                replaced.append(ActQuantWrapper(child))
            else:
                replaced.append(child)
        module.mm_projector = torch.nn.Sequential(*replaced)

def add_actquant_llamavmm(module, name='', layers=[torch.nn.Linear,
                                          ActQuantWrapper,
                                          transformers.models.falcon.modeling_falcon.FalconLinear]):
    if isinstance(module, ActQuantWrapper):
        return
    
    if type(module.multi_modal_projector) == torch.nn.Linear:
        module.multi_modal_projector = ActQuantWrapper(module.multi_modal_projector)
...
class Actfp8Quantizer(torch.nn.Module):
    ...
    def __init__(self):
        super(Actfp8Quantizer, self).__init__()
        self.register_buffer('maxq', torch.tensor(0))
        self.register_buffer('scale', torch.zeros(1))
        self.register_buffer('zero', torch.zeros(1))
        self.bits = 16
    ...
    def forward(self, x):
        ...
        if self.bits == 16:
            ...
            return symfp8_quant_dequant(x, self.scale, self.maxq).to(x_dtype)
        return asym_quant_dequant(x, self.scale, self.zero, self.maxq).to(x_dtype)
    ...
    def configure(self, bits, groupsize=-1, sym=False, clip_ratio=1.0):
        _, self.maxq = get_minq_maxqfp8(bits, sym)
        self.bits = bits
        self.groupsize = groupsize
        self.sym = sym
        self.clip_ratio = clip_ratio
    ...
    def find_params(self, x):
        ...
        if self.sym:
            ...
            self.scale = self.scale.unsqueeze(1).repeat(1, reshaped_x.shape[-1]).reshape(init_shape)
            self.zero = self.zero.unsqueeze(1).repeat(1, reshaped_x.shape[-1]).reshape(init_shape)
class Actfp8QuantWrapper(torch.nn.Module):
    '''
        This class is a wrapper for the activation quantization.
        We extract the FP features in the forward pass and quantize the rest using
        the self.quantizer object.
    ...
    def __init__(self, module:torch.nn.Linear):
        super(Actfp8QuantWrapper, self).__init__()
        assert isinstance(module, torch.nn.Linear)
        self.module = module
        self.weight = module.weight
        self.bias = module.bias
        self.quantizer = Actfp8Quantizer()
        self.out_quantizer = Actfp8Quantizer()
        self.register_buffer('had_K', torch.tensor(0))
        self._buffers['had_K'] = None
        self.K = 1
        self.online_full_had = False
...
class Weightfp8Quantizer(torch.nn.Module):
    '''From GPTQ Repo'''

    def __init__(self, shape=1):
        super(Weightfp8Quantizer, self).__init__()
        self.register_buffer('maxq', torch.tensor(0))
        self.register_buffer('scale', torch.zeros(shape))
        self.register_buffer('zero', torch.zeros(shape))
        ...
        if self.mse:
            ...
            for i in range(int(self.maxshrink * self.grid)):
                ...
                if self.sym:
                    scale1 = xmax1 / self.maxq
                    zero1 = torch.zeros_like(scale1)
                    q = symfp8_quant_dequant(x, scale1.unsqueeze(1), self.maxq)
                else:
...
def add_actquant_mmfp8(module, name='', layers=[torch.nn.Linear,
                                          Actfp8QuantWrapper,
                                          transformers.models.falcon.modeling_falcon.FalconLinear]):
    if isinstance(module, Actfp8QuantWrapper):
        return
    ...
    if type(module.mm_projector) == torch.nn.Sequential:
        ...
        for i, child in enumerate(module.mm_projector.children()):
            if type(child) in layers:
                replaced.append(Actfp8QuantWrapper(child))
            else:
                replaced.append(child)
        module.mm_projector = torch.nn.Sequential(*replaced)
    
def add_actquantfp8(module, name='', layers=[torch.nn.Linear,
                                          Actfp8QuantWrapper,
                                          transformers.models.falcon.modeling_falcon.FalconLinear]):
    if isinstance(module, Actfp8QuantWrapper):
        return
    for attr in dir(module):
        tmp = getattr(module, attr)
        if type(tmp) in layers:
            setattr(module, attr, Actfp8QuantWrapper(tmp))
        if type(tmp) == torch.nn.Sequential:
            replaced = []
            for i, child in enumerate(tmp.children()):
                if type(child) in layers:
        ...
        if type(tmp) == torch.nn.ModuleList:
            ...
            setattr(module, attr, torch.nn.ModuleList(replaced))
    for name1, child in module.named_children():
        add_actquantfp8(child, name + '.' + name1 if name != '' else name1, layers)
