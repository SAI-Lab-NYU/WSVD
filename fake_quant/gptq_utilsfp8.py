    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                gptq[name].quantizer = quant_utils.Weightfp8Quantizer()
    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                gptq[name].quantizer = quant_utils.Weightfp8Quantizer()
                gptq[name].quantizer.configure(
                    layer_weight_bits, perchannel=True, sym=layer_weight_sym, mse=False
                )
...
def rtn_fwrd(model, dev, args):
    ...
    for i in tqdm.tqdm(range(len(layers)), desc="(RtN Quant.) Layers"):
        ...
        for name in subset:
            ...
            quantizer = quant_utils.Weightfp8Quantizer()
            quantizer.configure(
...
@torch.no_grad()
def rtn_fwrdvit(model, dev, args):
    '''
    From GPTQ repo 
    TODO: Make this function general to support both OPT and LLaMA models
    '''
    ...
    for i in tqdm.tqdm(range(len(layers)), desc="(RtN vit Quant.) Layers"):
        layer = layers[i].to(dev)
